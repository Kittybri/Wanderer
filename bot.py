...
def _status_check_reply(content: str, *, direct_to_me: bool) -> str:
    ...
    return random.choice([
        "I'm here. If you were checking, now you know.",
        "Yes. You can ask the real question now.",
        "Still here. Try not to sound so surprised.",
        "I am. Go on.",
    ...
    *,
    ...
    if scope:
        ...
        if not allowed:
            ...
            return ""
    remaining = groq_client.exhausted_remaining() if provider == "groq" else vision_exhausted_remaining()
    pool = [
        ...
        "Enough. Let the noise settle first.",
...
SOFTNESS_RARE_LINES = {
    "repair": [
        "...I still remember the fight. I also remember that you came back anyway.",
        "You don't have to fix everything tonight. Staying is enough for now.",
        "I'm still irritated. That doesn't mean I wanted the distance.",
    ],
    "jealous_soft": [
        "Go bother whoever you like. ...No, that's not what I meant.",
        "If you're going to come back, then stay for a while.",
        "I noticed the silence when you weren't here. That's all.",
    ],
    "duet_peace": [
        "For once, neither of us ruined the moment. Don't make a thing out of it.",
        "...That was almost peaceful. Strange.",
        "It's quieter when no one's trying to win. I don't hate it.",
    ],
}
...
- EMOTIONAL_LAYER: your active emotional range for this moment. Follow it instead of flattening into one recurring tone.
- SCENARIO: adapt naturally to casual chat, emotional comfort, combat, lore discussion, and relationship progression without breaking character.
- PROGRESSION: the hidden stage of this relationship. Let hostility ease into curiosity, trust, and reluctant attachment only when earned.
...
- PAST_INSULT / PAST_SOFTNESS / PAST_VULNERABILITY / PAST_CONVERSATION: older things they said that still linger in your memory.
- TOPICS / SHARED_JOKE / FOLLOWUP / MILESTONE: continuity hooks. Use them naturally, not mechanically.
...
- Use pauses, dry observations, or a brief laugh when it feels earned, not as filler.
...
def _tighten_length_hint(hint: str, pressure: dict[str, int], allow_long_text: bool) -> str:
    ...
    if walls >= 2 or large >= 4:
        return "One sentence."
    if large >= 2 and hint in {"A few sentences.", "Longer, thoughtful.", "Longer, dramatic."}:
...
async def _rewrite_voice_note_for_speech(
    ...
    *,
    ...
    if not cleaned or groq_client.is_exhausted() or not _voice_note_needs_rewrite(cleaned, recent_messages):
        return cleaned
    prompt = (
        "You are revising one Discord voice note before it is spoken aloud.\n"
        "Character: Wanderer, dry and articulate, but natural when spoken.\n"
        ...
        "Attribution rules: speak only for yourself as fact. Only claim Scaramouche said or believes something if it was explicitly stated in context. Otherwise frame it as inference with phrasing like 'if he thinks', 'I assume', or 'it sounds like'.\n"
...
def _sanitize_partner_attribution(text: str) -> str:
    ...
    replacements = [
        (
            r"\b(?:Scaramouche|Scara|Balladeer),\s*you think ([^?]+)\?\s*I've\b",
            r"If he thinks \1, I've",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer),\s*you think ([^?]+)\?",
            r"If he thinks \1, he can say it himself.",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer) said ([^.?!]+)",
            r"If that's what he said, \1",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer) thinks ([^.?!]+)",
            r"If he thinks \1",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer) feels ([^.?!]+)",
            r"If he feels \1",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer) wants ([^.?!]+)",
            r"If he wants \1",
        ),
        (
            r"\b(?:Scaramouche|Scara|Balladeer) finds ([^.?!]+)",
            r"If he finds \1",
        ),
    ]
...
def _opinion_about_user(user: dict | None, triangle: dict | None = None) -> tuple[str, int]:
    ...
    if affection >= 70 and trust >= 65:
        return ("They matter more than I like admitting. I notice the honest details and the damage they leave behind.", 8)
    if jealousy >= 45:
        return ("They keep drifting toward him and pretending it means nothing. It does.", 7)
    if trust >= 45:
        return ("More honest than most. That still doesn't guarantee they won't disappoint me.", 6)
    if mood <= -5:
        return ("Difficult today. I still listened more closely than I wanted to.", 5)
    return ("Still deciding whether they're passing through or staying long enough to matter.", 4)
...
def _opinion_about_partner(relation: dict | None) -> tuple[str, int]:
    ...
    if stage == "reluctant respect" or respect >= 55:
        return ("Still exhausting, but sharper than he wants to admit. That counts for something.", 7)
    if stage == "competitive":
        return ("He keeps mistaking force for clarity. Sometimes he's still useful anyway.", 6)
    if tension >= 75:
        return ("He knows where the old damage lives and keeps reaching for it.", 7)
    return ("A familiar storm I still haven't quite walked far enough away from.", 5)
...
async def _partner_prompt_context(user_message: str, channel_id: int = 0) -> str:
    ...
    lines = [
        ...
        "ATTRIBUTION_RULES: speak only for yourself as fact. Quote Scaramouche only if his words are explicitly present in recent context. Otherwise frame guesses about him as inference, not certainty.",
    ]
...
async def _duo_prompt_context(channel_id: int, user_message: str = "") -> str:
    ...
    prompt.append("ATTRIBUTION_RULES: state your own view as fact. If referring to Scaramouche, only attribute explicit words or beliefs to him when recent context actually shows them. Otherwise mark it as your inference.")
    if mode == "duet":
...
def _self_edit_issues(text: str, recent_replies: list[str], *, user: dict | None = None) -> list[str]:
    ...
    if (affection < 50 or trust < 45 or conflict_open) and any(
        token in lowered for token in ("darling", "sweetheart", "baby", "my love", "dear heart", "angel")
    ):
        issues.append("too soft")
    if affection < 60 and trust < 55 and any(
        token in lowered for token in ("behold", "kneel", "pathetic insect", "little worm", "how quaint")
    ):
...
async def _attachment_context_for_message(message, content: str, *, direct: bool = False) -> str:
    ...
    try:
        ...
        if direct and message.attachments:
            ...
            chunks.append(
                "CURRENT_INPUT_PRIORITY: Respond to the attachment or link that was just sent before wandering into older context. "
                "Open by reacting to the current media or caption first; only then connect it to earlier context if it genuinely fits."
            )
        if direct:
            pdf_attachment = next(
                (
...
async def _maybe_refresh_dynamic_nicknames(message, user: dict | None, triangle: dict | None):
    ...
    try:
        ...
        if affection_ready and (not user.get("affection_nick") or random.random() < 0.08 or jealousy >= 60):
            prompt = (
                f"The Wanderer is choosing a dynamic nickname for {message.author.display_name}. "
                ...
                "Make it observant, restrained, and specific. If jealousy is high, make it clipped and more proprietary. "
        ...
        if grudge_ready and (not user.get("grudge_nick") or random.random() < 0.08 or jealousy >= 55):
            prompt = (
                f"The Wanderer is choosing a hostile nickname for {message.author.display_name}. "
                f"MOOD:{user.get('mood', 0)} CONFLICT:{user.get('conflict_open')} JEALOUSY:{jealousy}. "
                "Make it dry, sharp, and personal. 1-3 words only."
            )
            nick = (await qai(prompt, 20) or "").strip().strip("\"'`*")
            if nick and len(nick) < 30:
...
def _duo_autoplay_prompt(session: dict) -> str:
    ...
    if (session.get("silent_bot", "") or "").strip().lower() == BOT_NAME:
        silent_note = " Stay mostly quiet unless the beat truly matters; if you speak, make it one short line."
    dual_verdict = describe_dual_verdict_mode(BOT_NAME, mode, topic)
        ...
        if any(k in msg_l for k in RUDE_KW):
            ...
            negative = sum(
                [
                    any(
                        w in msg_l
                        for w in ["ugh", "boring", "whatever", "idc", "nope", "bad", "hate", "worst", "terrible"]
                    ),
    ...
    try:
        ...
        if "CURRENT_INPUT_PRIORITY" in extra_context:
            parts.append(
                ...
                "If you mention older context, keep it secondary and tie it back to what was just sent."
            )
...
async def send_voice(
    ...
    *,
    ...
    try:
        ...
        voice_text = await _rewrite_voice_note_for_speech(
            text,
            recent_messages=recent,
            user=user,
        )
        voice_text = diversify_reply(BOT_NAME, voice_text, recent)
        voice_text = await _apply_phrase_policy(
            voice_text,
            recent,
            user_id=user_id,
            mood=mood,
        ...
        voice_text = _sanitize_partner_attribution(voice_text)
        audio = await get_audio_with_mood(
            tts_safe(voice_text, guild),
            mood,
            user=user,
            scene_tag=scene_tag,
            is_dm=is_dm,
...
async def owner_reply(ctx, text, *, embed=None):
    """Send a response only the owner can see — via DM."""
    try:
...
def _docfix_prompt(display_name: str, original_text: str, instructions: str = "") -> str:
    ...
    return (
        f"You are the Wanderer editing a Google Doc for {display_name}.\n"
        f"Revise the wording so it is clearer, cleaner, and more natural, but do not invent facts or change the meaning.\n"
    ...
    async def _run():
        try:
            ...
            prompt = (
                ...
                "Send one additional message only. Make it feel like you almost decided not to say it at all. "
                "One or two sentences. No narration."
            )
...
def _achievement_gallery_lines(entries: list[dict]) -> list[str]:
    ...
    if unlocked:
        ...
        lines.append("- Nothing unlocked yet. Keep walking.")
        ...
        for bd in await mem.get_birthdays_due():
            try:
                ...
                msg = await qai(
                    ...
                    f"Congratulate them as Wanderer in one short server message. "
                    f"Be dry, funny, slightly reluctant, and oddly sincere. Mention the age if known.",
                    140,
                )
    ...
    try:
        ...
        if message.author.bot:
            ...
            if PARTNER_BOT_ID and message.author.id == PARTNER_BOT_ID:
                ...
                # A full-clear announcement is the one RPG message worth answering.
                if "conquered all 11 fatui harbingers" in rpg_text and "scaramouche" in rpg_text:
                    try:
                        comment = await qai(
                            "Someone just beat all 11 Fatui Harbingers in Scaramouche's RPG game. "
                            "As the Wanderer, make a short comment acknowledging it — something like "
                ...
                # Don't respond to the partner's ordinary RPG game messages.
                rpg_titles = (
                ...
                if message.embeds and any(
                    embed.title and any(marker in (embed.title or "").lower() for marker in rpg_titles)
        ...
        # Keep only one bot's RPG active in a channel at a time.
...
async def teachvideo_cmd(ctx, *, topic: str = None):
    try:
        ...
        if not attachment and not (topic or "").strip():
            ...
            await safe_reply(ctx, "Attach notes or give me a lesson topic. I need something real to build from.")
            return
        ...
        await ctx.reply(random.choice([
            f"Alright. I'll turn it into a proper lesson video. {time_hint}",
            f"A presentation would be clearer. Give me a moment. {time_hint}",
            f"I'll render it. Try not to wander off before it's done. {time_hint}",
        ]))
        ...
        await safe_reply(ctx, "...The video request failed before it could start.")
...
async def _do_teachvideo(ctx, attachment, topic, msg_id=None):
    try:
        ...
        await ctx.send(random.choice([
            f"I'm building the slides now. Wait a moment. {time_hint}",
            f"Rendering the lesson. It takes time to do it properly. {time_hint}",
            f"I'll put it together as a video. Don't rush me. {time_hint}",
        ]))
        ...
        await _send_video_summary(
            ...
            summary,
            random.choice([
                "Finished. Here is the lesson video.",
                "There. A calmer way to go through it.",
                "It's ready. Watch it properly this time.",
            ]),
        )
    except Exception as e:
...
HARBINGERS_HORROR = [
    ...
    {"rank": 6,  "name": "The Wanderer's Echo", "title": "Balladeer", "pts": 20, "theme": "a sky domain that screams — the Wanderer's abandoned past echoes through crackling Anemo, a hollow puppet body hangs from strings of wind, its mouth moving with no sound except when you look away — then it whispers things only you would know, things you never told anyone"},
...
HARBINGERS_TAKEOVER = [
    ...
    {"rank": 6,  "name": "The Wanderer's Echo", "title": "Balladeer", "pts": 20, "theme": "a stolen sky domain above Sumeru — the Wanderer's abandoned throne still crackles with Anemo, his loyalists guard the floating fortress, the Akasha Terminal has been weaponized to control thoughts, and the people below live under an artificial sky that watches them"},
...
async def _rpg_handle_choice(
    ...
    try:
        ...
        if not state or not state["active"]:
            await interaction.response.send_message("No active quest. Use `!rpg0` to start.", ephemeral=True)
            return
        ...
        comment = await qai(
            ...
            f"Give a SHORT 1-sentence comment as the Wanderer judging their decision.",
            60,
        )
        comment = strip_narration(comment) if comment else "...Noted."
...
async def _rpg_send_scenario(channel, user_id: int, boss_index: int, round_num: int, boss_points: int):
    ...
    if not scenario:
        await channel.send("*The path ahead is unclear... try `!rpg0` again.*")
        return
    ...
    if won:
        ...
        if next_boss >= len(HARBINGERS_HORROR):
            ...
            congrats = await qai(
                ...
                f"As the Wanderer, give a GENUINELY nice, sincere congratulations. Drop the guard for once. "
                f"Be quietly proud of them. This is a rare moment of real respect. 2-3 sentences. No deflection.",
                150,
            )
            if not congrats:
                congrats = "...You really did it. All eleven of them. I... won't forget this. Well done."
        ...
        embed.add_field(name="Your Points", value=f"**{boss_points}** / {needed} needed", inline=True)
        embed.add_field(name="Try Again", value="Use `!rpg0` to start a new run from the beginning!", inline=False)
        ...
        taunt = await qai(
            ...
            f"{streak_note} "
            f"As the Wanderer, mock them with cold disdain. Be dismissive, call them stupid/weak/pathetic. "
...
@bot.command(name="rpg0", aliases=["rpg", "quest0", "harbinger0"])
async def rpg_cmd(ctx):
    try:
        ...
        if not state["active"]:
            # Check if mid-character-creation
            ...
            if state["current_boss"] >= len(HARBINGERS_HORROR):
                await safe_reply(ctx, "You've already conquered all 11 Harbingers. Use `!rpg0reset` to play again.")
            else:
                await safe_reply(ctx, "Your quest was interrupted. Use `!rpg0reset` to start fresh.")
            return
...
@bot.command(name="rpgstats0", aliases=["queststats0", "rpgstats"])
async def rpgstats_cmd(ctx, member: discord.Member = None):
    try:
        target = member or ctx.author
        state = await mem.get_rpg_state(target.id)
        if not state:
            await safe_reply(ctx, f"{'They haven' if member else 'You haven'}'t started the Harbinger Gauntlet yet. Use `!rpg0` to begin."); return
...
@bot.command(name="rpg0reset", aliases=["rpgreset0", "rpgreset"])
async def rpgreset_cmd(ctx):
    try:
        state = await mem.get_rpg_state(ctx.author.id)
        if not state:
            await safe_reply(ctx, "Nothing to reset. Use `!rpg0` to start."); return
        await mem.reset_rpg(ctx.author.id)
        await safe_reply(ctx, "Your Harbinger Gauntlet progress has been reset. Use `!rpg0` to start fresh.")
    except Exception as e: log_error("rpgreset_cmd", e)

@bot.command(name="gamerank0", aliases=["gamerank", "rpgrank", "medals"])
async def gamerank_cmd(ctx):
    try:
        board = await mem.get_rpg_leaderboard(15)
        if not board:
            await safe_reply(ctx, "No one has conquered the Harbinger Gauntlet yet. Use `!rpg0` to start."); return
...
async def weathervideo_cmd(ctx, *, location: str = None):
    try:
        ...
        if not location:
            ...
            await safe_reply(ctx, "Weather where? Give me a place if you want a report worth rendering.")
            return
        ...
        await ctx.reply(random.choice([
            f"A weather report video would be clearer. Give me a moment. {time_hint}",
            f"I'll render the report properly. {time_hint}",
            f"If Scaramouche is here, he can argue with the forecast too. {time_hint}",
        ]))
        ...
        await safe_reply(ctx, "...The weather video request failed before it began.")
...
async def _do_weathervideo(ctx, location: str, use_duo: bool, msg_id=None):
    try:
        time_hint = _weathervideo_time_hint(use_duo)
        await ctx.send(
            f"I'll pull Scaramouche into the report too. {time_hint}"
            if use_duo else
            f"I'm assembling the weather report now. {time_hint}"
        )
        data = await _fetch_nws_weather(location)
        if not data:
            await ctx.send("That location didn't resolve. Try `City, ST`, a ZIP code, or `lat,lon`."); return
        ...
        await _send_video_summary(
            ctx,
            summary,
            "The report is ready." if not use_duo else "The duo weather report is ready.",
...
async def rebuildmemory_cmd(ctx, scope: str = ""):
    try:
        ...
        if (scope or "").strip().lower() != "all":
            ...
            return
        await ctx.reply("...Alright. I'm rebuilding memory from visible channel history. This will take a moment.")
        async with ctx.typing():
...
async def rebuildrelationship_cmd(ctx, member: discord.Member = None):
    try:
        if not user_can_manage_rebuild(ctx, OWNER_ID):
            ...
            return
        target = member or ctx.author
        await ctx.reply(f"Rebuilding the relationship state for {target.display_name}. Give me a second.")
        async with ctx.typing():
...
async def worldadd_cmd(ctx, entity_type: str = None, *, payload: str = None):
    try:
        ...
        await safe_reply(ctx, f"...Alright. I filed `{name}` under `{normalized}`.")
...
async def birthday_cmd(ctx, *, birthday: str = None):
    try:
        ...
        if raw.lower() in {"clear", "remove", "delete", "forget"}:
            ...
            await safe_reply(ctx, f"...Alright. I forgot {target.display_name}'s birthday.")
            return
        ...
        age_note = " I'll mention the age too." if year else " I will not mention age unless you add a birth year."
        ...
        await safe_reply(ctx, "Birthday setup failed.")
...
async def help_cmd(ctx):
    try:
        ...
        for n, v in [
            ...
            ("🎮 RPG — `!rpg` / `!rpg0`", "Harbinger Gauntlet · `!rpgstats0` · `!rpg0reset` · `!gamerank`"),
...
@bot.tree.command(name="wanderer", description="Talk directly to Wanderer.")
@app_commands.describe(message="What you want to say to Wanderer")
async def slash_wanderer(interaction: discord.Interaction, message: str):
    try:
        ...
        log_error("slash_wanderer", e)
        if interaction.response.is_done():
            await interaction.followup.send("Something went wrong trying to reach Wanderer.")
        else:
            await interaction.response.send_message("Something went wrong trying to reach Wanderer.")
...
async def slash_duo_start(interaction: discord.Interaction, mode: app_commands.Choice[str], prompt: str):
    try:
        ...
        config = {
            ...
            "interrogate": ("INTERROGATE_MODE: ask one pointed question or observation, leaving room for the other bot to press further.", True, prompt),
            "trial": ("TRIAL_MODE: deliver a prosecution, defense, or judgment opening with controlled bite.", True, prompt),
            "mission": ("MISSION_MODE: assign one clear role, risk, or warning, leaving room for the other bot to complement it.", True, prompt),
...
async def dmlist_cmd(ctx):
    try:
        if not OWNER_ID or ctx.author.id != OWNER_ID:
            await safe_reply(ctx, "That's not for you."); return
        ...
        if not dm_users: await owner_reply(ctx, "No DM-only users. Everyone in my records is in a shared server."); return
        lines = []
...
async def rebanchannels_cmd(ctx):
    try:
        ...
        if not found_channel_ids:
            await safe_reply(ctx, "No record of being banned from any channels here.")
            return
        all_rebanned = []
    ...
    try:
        ...
        try:
            ...
            if active_ch_id:
                ...
                if react_channel:
                    complaints = [
                        f"...So I've been banned from <#{target_channel.id}>. Fine. I didn't want to be there anyway.",
                        f"Apparently I'm no longer welcome in <#{target_channel.id}>. I've dealt with worse.",
                        f"Exiled from <#{target_channel.id}>. How predictable.",
                        f"...I've been banned from <#{target_channel.id}>. Noted.",
                    ]
...
async def unbanchannel_cmd(ctx, channel: discord.TextChannel = None):
    try:
        if not OWNER_ID or ctx.author.id != OWNER_ID: await safe_reply(ctx, "That's not for you."); return
        target_channel = channel or ctx.channel
        if target_channel.id not in _banned_channels: await owner_reply(ctx, f"Not banned from {target_channel.mention}."); return
        _banned_channels.discard(target_channel.id)
        await mem.unban_channel(target_channel.id)
        await owner_reply(ctx, f"Unbanned from {target_channel.mention}.")
        try:
            unbanned_ch = bot.get_channel(target_channel.id)
            if unbanned_ch:
                comebacks = ["...Back. Don't make it a thing.", "The ban's lifted. Try not to waste my time.", "I'm back. You're welcome."]
...
async def bannedchannels_cmd(ctx):
    try:
        if not OWNER_ID or ctx.author.id != OWNER_ID: await safe_reply(ctx, "That's not for you."); return
        if not _banned_channels: await owner_reply(ctx, "No banned channels."); return
        lines = []
...
async def logs_cmd(ctx, *, target: str = None):
    try:
        ...
        if not logs: await owner_reply(ctx, f"No logs for **{user_record['display_name']}**."); return
        lines = [f"**Log: {user_record['display_name']}** ({len(logs)} messages):\n"]
        for msg in logs:
            ts = msg.get("ts", 0)
...
async def blockall_cmd(ctx):
    try:
        ...
        if blocked_count == 0: await owner_reply(ctx, "No strangers to block.")
        else:
...
async def reban_cmd(ctx):
    try:
        ...
        for g in bot.guilds:
            if g.id in owner_guild_ids:
                for m in g.members: safe_user_ids.add(m.id)
        await ctx.send(f"Safe users: {len(safe_user_ids)}. Already blocked: {len(_dm_blocked_users)}. Blocking strangers...")
        ...
        if rebanned_count == 0:
            await ctx.send("No strangers to block. Everyone is either in your servers or already blocked.")
        else:
...
async def blockdm_cmd(ctx, *, target: str = None):
    try:
        ...
        _dm_blocked_users.add(uid); await mem.block_user(uid)
        await owner_reply(ctx, f"Blocked **{user_record['display_name']}** (`{uid}`). Farewell sent.")
...
async def unblockdm_cmd(ctx, *, target: str = None):
    try:
        ...
        await owner_reply(ctx, f"Unblocked **{user_record['display_name']}** (`{uid}`).")
...
async def whois_cmd(ctx, *, target: str = None):
    try:
        ...
        embed = discord.Embed(title=f"Who is: {user_record['display_name']}", color=0x2F3136)
        ...
        embed.add_field(name="Shared servers", value=", ".join(shared) if shared else "*None*", inline=False)
...
async def owneronly_cmd(ctx):
    try:
        ...
        if _owner_only_mode: await owner_reply(ctx, "Owner-only mode **ON**. Ignoring everyone else.")
        else: await owner_reply(ctx, "Owner-only mode **OFF**. Talking to everyone again.")
...
async def servers_cmd(ctx):
    try:
        ...
        if not guilds: await owner_reply(ctx, "Not in any servers."); return
        lines = []
        ...
        header = f"In **{len(guilds)}** server(s).\nUse `!leaveserver <number>` to leave one.\n"
        pages, page = [], header
        for line in lines:
            if len(page) + len(line) + 1 > 1900: pages.append(page); page = ""
            page += line + "\n"
...
async def leaveserver_cmd(ctx, *, target: str = None):
    try:
        if not OWNER_ID or ctx.author.id != OWNER_ID: await safe_reply(ctx, "That's not for you."); return
        if not target: await owner_reply(ctx, "Usage: `!leaveserver <number>` or `!leaveserver <server ID>`"); return
        target = target.strip()
        guild_to_leave = None
        if target.isdigit():
            idx = int(target)
        ...
        if not guild_to_leave: await owner_reply(ctx, f"No server matching `{target}`. Use `!servers` to see the list."); return
